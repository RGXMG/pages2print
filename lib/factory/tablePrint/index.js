...
export default class TablePrint {
  ...
  createContent(tableOptions) {
    ...
    for (let i = 0; i < tableMeta.length; i++) {
      ...
      const { nextTableMarginSize, trIsNextPage, rewrittenTrMeta } =
        this.trOutOfBoundsGuardInstance.tryCreateTrElement2Table(trMeta, i, tableOptions);
      ...
      if (trIsNextPage) {
        ...
        // 尝试合并待提交的新tr元数据与当前越界的新的tr数据
        const combineWaitCommitNewTrMeta =
          this.trOutOfBoundsGuardInstance.getWaitCommitNewTrMeta(rewrittenTrMeta);

        // 再次处理待提交的数据
        // 1. 添加数据到tableMeta中
        // 2. 循环处理，处理某个td的高度超过2页以上
        tableMeta.splice(i + 1, 0, combineWaitCommitNewTrMeta);
      } else {
        table.push();
      }
    }
