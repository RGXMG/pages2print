...
export default class TrOutOfBoundsGuard {
  ...
  getWaitCommitNewTrMeta(rewriteTrMeta) {
    ...
    this.waitCommitNewTrMeta.forEach(
      (tdMeta, $index) =>
        (newTrMeta[$index] = {
          ...tdMeta,
          // 需要根据渲染次数得到最新的tdMeta
          rowspan: tdMeta.rowspan > 1 ? tdMeta.rowspan - times : 1,
        })
    );
    // 重置
  ...
  tryCreateTrElement2Table(trMeta, $i, tableMeta) {
    ...
    // 重写的tr元数据：
    // 1. 该tr越界，该tr不会渲染，且该rewriteTrMeta会被改写
    // 2. 该tr内部的越界，该rewriteTrMeta会被改写
    ...
    // 1. td如果超出阈值，会被分解
    // 2. 分解后的td所在的tr不会发生越界
    const { trElement, rewrittenTrMeta, rewrittenTrElement, isRewritten } =
      this.createTrElement(rewriteTrMeta);

    // 2. 该tr内部的越界，该rewriteTrMeta会被改写
    if (isRewritten) {
      rewriteTrMeta = rewrittenTrMeta;
    }

    // 验证是否超出边界
    const [isOutOfBounds, trBottom2Top] = this.validateOutOfBounds(trElement, rewriteTrMeta);
    ...
    if (isOutOfBounds) {
      ...
      const {
        ...
        newTrElement: r,
      } = this.handleOutOfBounds(rewriteTrMeta, $i, tableMeta);
      nextTableMarginSize = n;
      rewriteTrMeta = t;
      rewriteTrElement = r;
    }
  ...
  handleOutOfBounds(trMeta, $i, tableMeta) {
    ...
    const nextTableMarginSize = this.pageHeightPixelSize - this.prevTrElementBoundingClientTop;
  ...
  validateOutOfBounds(trElement, trMeta) {
    ...
    const trBottom2Top = top + height + this.printPageContainerElement.scrollTop;
  ...
  createTrElement(trMeta) {
    ...
    let trIsNotValid = false;
    // 重写过的tr元数据
    let rewrittenTrMeta = [...trMeta];
    for (let index = 0; index < trMeta.length; index++) {
      const tdMeta = trMeta[index];
      const { isOutOfBounds, beDecomposed, rewriteTdMeta, rewriteTdElement, rebuildTdMeta } =
        this.tdOutOfBoundsGuardInstance.tryCreateTdElement2Tr(tdMeta, index, trElement);

      if (!trBeDecomposed && beDecomposed) trBeDecomposed = beDecomposed;
      else if (!trIsNotValid && isOutOfBounds && !beDecomposed) trIsNotValid = true;
