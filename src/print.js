...
function getA4PagePixelHeight() {
  ...
  const height = iframe.contentDocument.defaultView.getComputedStyle(page).height;
...
function createOutOfBoundsGuard() {
  ...
  const getOutOfBounds = (tr, meta) => {
    ...
    console.log(meta, 'top:::', top, 'height', height, pageHeight, top + height > pageHeight);
    return res;
  };
...
function toPrint() {
  ...
  json.forEach((trMeta, index, json) => {
    const { isOutOfBounds, strandedHeight, newTrMeta } = getOutOfBoundsCompareResult(
      trMeta,
      index,
      json
    );
